...
/* wangzhengyong add for re-connect func begin */
#define SD_SHEEP_PROTO_VER 0x09
/* wangzhengyong add for re-connect func end */
	...
	while (s->fd < 0) {
		...
		while (retry--) {
			...
			if (s->fd < 0) {
				...
				DPRINTF("reconnect finished, host index %d, addr %s \n", s->host_index, s->host_spec);
				break;
			}
		}
	}
...
/* wangzhengyong add for re-connect func begin */
static int sd_get_nodes(BDRVSheepdogState *s)
{
	int ret;
    ...
    closesocket(fd);
    if (ret) {
        error_setg_errno(&local_err, -ret, "failed to send a request to the sheep");
		goto out;
    }
	...
	if (rsp->result == SD_RES_SUCCESS) {
		...
		error_setg_errno(&local_err, -ret, "failed to get nodes");
		ret = rsp->result;
		g_free(s->hosts);
	}
